...
function App() {
  ...
  // La variable hasMyTeamDeclaredBelote est maintenant supprimée car obsolète.
    ...
    <div style={{ padding: '20px' }}>
      ...
      {gameState?.teams.map(team => (
        <div key={team.name} style={{float: 'right', marginLeft: '20px', border: '1px solid grey', padding: '10px', minWidth: '150px', textAlign: 'center'}}>
          ...
          {/* Correction pour afficher le bon message de belote */}
      ...
      {gameState?.phase === 'bidding' && ( <div style={{ margin: '20px 0', padding: '10px', border: '2px solid red' }}><h3>Phase de prise (1er Tour)</h3>{gameState.currentPlayerTurn === socket.id && ( <div><button onClick={() => handleBid('take')}>Prendre</button><button onClick={() => handleBid('pass')}>Passer</button></div> )}</div> )}
      {gameState?.phase === 'bidding_round_2' && ( <div style={{ margin: '20px 0', padding: '10px', border: '2px solid orange' }}><h3>Phase de prise (2ème Tour)</h3>{gameState.currentPlayerTurn === socket.id && ( <div><p>Choisissez une couleur :</p>{SUITS.filter(suit => suit !== gameState.biddingCard?.suit).map(suit => (<button key={suit} onClick={() => handleBid(suit)}>{suit}</button>))}<button onClick={() => handleBid('pass')}>Passer</button></div> )}</div> )}
      ...
      {gameState?.phase === 'end' && ( <div style={{ margin: '20px 0', padding: '10px', border: '2px solid purple' }}><h3>Fin de la manche</h3>{gameState.teams.map(team => ( <div key={team.name}><h4>{team.name} (Score Total: {team.score}){team.name === gameState.takerTeamName && ( <>{gameState.contractResult === 'succeeded' && <span style={{color: 'green', marginLeft: '10px'}}> (Contrat réussi)</span>}{gameState.contractResult === 'failed' && <span style={{color: 'red', marginLeft: '10px'}}> (Contrat chuté)</span>}</> )}</h4><p>Points de la manche: {gameState.roundPoints?.[team.name] ?? 0}</p></div> ))}{gameState.players[0].id === socket.id && ( <button onClick={handleNextHand}>Manche suivante</button> )}</div> )}
      {gameState?.phase === 'game_over' && ( <div style={{ margin: '20px 0', padding: '10px', border: '2px solid black', backgroundColor: 'gold' }}><h2>Partie Terminée !</h2><h3>Vainqueur : {gameState.teams.find(t => t.score >= WINNING_SCORE)?.name}</h3>{gameState.teams.map(team => ( <div key={team.name}><h4>{team.name} - Score Final : {team.score}</h4></div> ))}{gameState.players[0].id === socket.id && ( <button onClick={handleNewGame}>Nouvelle Partie</button> )}</div> )}
      
      {me.hand.length > 0 && <h3>Votre main ({me.hand.length} cartes) :</h3>}

      {/* ############ BLOC DE BOUTONS CORRIGÉ ############# */}
